...
function yaml_config {
    sed -e "s/\${PORT}/$1/g" \
        ...
        -e "s/\${HOSTS}/$6/g" \
        $WORKING_DIR/yaml files/fabric-ca-server-config.yaml | sed -e $'s/\\\\n/\\\n          /g'
}
...
function yaml_docker {
    sed -e "s/\${NAME}/$1/g" \
        ...
        -e "s/\${PASSWORD}/$4/g" \
        -e "s/\${LISTENINGPORT}/$5/g" \
        $WORKING_DIR/yaml files/ca-docker.yaml | sed -e $'s/\\\\n/\\\n          /g'
}
...
function docker() {
    ...
    if [ ! -d "${FILE1,,}" ]; then
        >${FILE}/${NAME}.yaml
        echo "$(yaml_docker ${ORG} ${PORT} ${USERNAME} ${PASSWORD} ${LISTENINGPORT})" > $FILE/${NAME}.yaml
    fi
