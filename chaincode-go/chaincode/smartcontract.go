...
func (s *SmartContract) CreateAsset(ctx contractapi.TransactionContextInterface, id string, color string, size int, owner string, appraisedValue int, category string, value string, latitude float64, longitude float64, namestamp string) error {
	...
	asset = hash(asset)
	asset = gps(asset, latitude, longitude, namestamp)
	code(asset)
	qr := asset.QR
	if err != nil {
		return err
...
func (s *SmartContract) UpdateGPS(ctx contractapi.TransactionContextInterface, id string, latitude float64, longitude float64, namestamp string) error {
	asset, err := s.ReturnAsset(ctx, id)
	if err != nil {
		return err
	}
...
func (s *SmartContract) ReturnAsset(ctx contractapi.TransactionContextInterface, id string) (Asset, error) {
	qrcode, err := s.GetQR(ctx, id)
	if err != nil {
	...
	if err != nil {
		return Asset{}, fmt.Errorf("failed to decode asset: %v", err)
	}
	return asset, nil
}
...
func (s *SmartContract) GPSsearch(ctx contractapi.TransactionContextInterface, name string) ([]*Asset, error) {
	// range query with empty string for startKey and endKey does an
	// open-ended query of all assets in the chaincode namespace.
	resultsIterator, err := ctx.GetStub().GetStateByRange("", "")
	if err != nil {
	...
	for resultsIterator.HasNext() {
		...
		for i := 0; i < t.NumField(); i++ {
			if t.Field(i).Name == "GPS" {
				for _, gps := range asset.GPS {
					if gps.Name == name {
					assets = append(assets, &asset)
					break
					}
...
func code(newasset Asset) {
	...
	err := qrcode.WriteFile(newasset.QR, qrcode.High, 256, fmt.Sprintf("/tme/%s.png", newasset.ID))
	if err != nil {
...
func gps(newasset Asset, latitude float64, longitude float64, namestamp string) Asset {
	...
	newasset.GPS = append(newasset.GPS, gps)
