...
function yaml_orderer {
    sed -e "s/\${PORT}/$1/g" \
        ...
        -e "s/\${OPERADDRESS}/$8/g" \
        -e "s/\${ADMIN_PORT}/$9/g" \
        $WORKING_DIR/yaml files/orderer.yaml | sed -e $'s/\\\\n/\\\n          /g'
}
...
function yaml_orderer_docker {
    sed -e "s/\${PORT}/$1/g" \
        ...
        -e "s/\${OPERADDRESS}/$5/g" \
        -e "s|FILE|$6|g" \
        $WORKING_DIR/yaml files/orderer-docker.yaml | sed -e $'s/\\\\n/\\\n          /g'
}
...
function config_orderer_docker() {    
        > ${WORKING_DIR}/organizations/${ORG,,}/orderers/${NAME}/orderer-docker.yaml
