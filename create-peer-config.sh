...
function yaml_core {
    sed -e "s/\${PORT}/$1/g" \
        ...
        -e "s|MSPATH|$9|g" \
        -e "s/\${MSPID}/${10}/g" \
        -e "s/\${OPERADDRESS}/${11}/g" \
        $WORKING_DIR/yaml files/core.yaml | sed -e $'s/\\\\n/\\\n          /g'
}
...
function yaml_couch {
    sed -e "s/\${COUCHPORT}/$1/g" \
        -e "s/\${ORG}/$2/g" \
        -e "s/\${NAMEPEER}/$3/g" \
        $WORKING_DIR/yaml files/couch.yaml | sed -e $'s/\\\\n/\\\n          /g'
}
...
function yaml_peer {
    sed -e "s/\${PORT}/$1/g" \
        ...
        $WORKING_DIR/yaml files/peer.yaml | sed -e $'s/\\\\n/\\\n          /g'
}


function config_couch() {    
        > ${WORKING_DIR}/organizations/${ORG,,}/peers/${NAMEPEER}/couch.yaml
        echo "$(yaml_couch ${COUCHPORT} ${ORG} ${NAMEPEER} )" > ${WORKING_DIR}/organizations/${ORG,,}/peers/${NAMEPEER}/couch.yaml
}

function config_core() {    
        > ${WORKING_DIR}/organizations/${ORG,,}/peers/${NAMEPEER}/core.yaml
        echo "$(yaml_core ${PORT} ${NAMEPEER} ${ORG} ${CHAINCODE_PORT} ${ADDRESSES} ${LEADER} ${FILE} $LEDGER $MSPATH $MSPID ${OPERADDRESS})" > ${WORKING_DIR}/organizations/${ORG,,}/peers/${NAMEPEER}/core.yaml
}
